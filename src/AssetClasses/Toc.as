package src.AssetClasses
{
	...
	public class Toc extends MovieClip
	{
		...
		public function buildIndex(m:CourseModel,tc:uint,pc:uint,oc:uint,dc:uint,st:Settings,hc:uint):void
		{
			...
			for each (var item in m.topicAndPages) {
			   // trace("I: " + item.localName());
			    if (item.localName().toLowerCase() == "page" && item.@nonNavPage.toString().toLowerCase() != "true" && item.@isTocEntry.toString().toLowerCase() != "false")
			    {
		...
		public function createPage(p:XML,nLevel:int,bSelectable:Boolean):void
		{
			...
			if (_visited_array[_nIndex] == undefined)
			{
				// we do not, initialize it to not visited
				_visited_array[_nIndex] = "0";
			} else if (_visited_array[_nIndex] == "1"){
				this_mc.visited = "1";
			}
			
			// see if we have data in the selectable array
			if (_selectable_array[_nIndex] == undefined)
			{
				// we do not, initialize it to the selectable value
				if (bSelectable)
					_selectable_array[_nIndex] = "1";
				else
					_selectable_array[_nIndex] = "0";
			}
			
			// create a user-defined property to remember if this node was visited
			this_mc.visited = _visited_array[_nIndex];
			
			// see if it has been visited
			if (_visited_array[_nIndex] != "0")
			{
			...
			this_mc.selectValue = _selectable_array[_nIndex];
			
			// see if this page is selectable
			if (_selectable_array[_nIndex] == "0")
			{
		...
		public function createTopic(t:XML,nLevel:int,bSelectable:Boolean):void
		{
			...
			for each (var it in t.elements()) {
			    //trace("I: " + item.localName());
			    if (it.localName().toLowerCase() == "page" && it.@nonNavPage.toString().toLowerCase() != "true" && it.@isTocEntry.toString().toLowerCase() != "false")
			    {
			    	//trace("----" + it);
		...
		public function parseQuiz(quiz:XML):void
		{
			trace("creating quiz");
			for each (var it in quiz.elements()) {
				//trace("I: " + item.localName());
				/*if (it.localName().toLowerCase() == "page" && it.@nonNavPage.toString().toLowerCase() == "false")
				{
		...
		public function markComplete(idx:int):void //Called from IndexView to mark the page completed
		{
			...
			if (selected_mc.visited == "0")
			{
				...
				_visited_array[selected_mc.index] = "1";
